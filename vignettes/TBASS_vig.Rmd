...
where the subscript $c$ denotes the candidate move step of the algorithm (a birth step in this case).\newline
...
\begin{equation}
\boldsymbol{\beta} | \cdot \sim \mathcal{N}\left\lbrace \boldsymbol{\hat\beta}, \left(X'V^{-1}X + \tau^{-2}I\right) \right\rbrace
\end{equation}
...
```{r}
fig <- fig + 1
plot(sqrt(tb$s2[-burn]), type = "l",
    main = paste0("Figure ", fig, ": Trace plot of sigma"))
```

From this plot, we see that our simulated $\sigma$ values are close to $1$, which matches our true value of $\sigma$ when we generated our data.

We then plot our $\frac{1}{V_i}$ values to see if the outliers are accounted for.
