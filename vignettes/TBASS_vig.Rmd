...
Equation (\ref{1}) is used to calculate the ijth element of the ${\bf X}$ matrix of basis functions, where $s_i \in \lbrace-1,1\rbrace$, $t_i$ is called a **knot** and $[a]_{+} = \max(0, a)$.
...
\boldsymbol{\hat{\beta}} = 
\frac{1}{\sigma^2}
\left( \frac{1}{\sigma^2} {\bf X'VX} + {\tau^{-2}}{{\bf I}} \right)^{-1} {\bf X'} {\bf V}~{{{\bf y}}}
\end{equation}
...
where $D$ represents the data, $\theta'$ represents the candidate model parameters and $\theta$ represents the current model parameters, $L$ is the likelihood, $p$ is the prior, and $S$ is the proposal to jump from one model to another. 
