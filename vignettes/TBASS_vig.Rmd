...
The R package TBASS is an extension of the BASS package created by Francom and Sansó (2019). The package is used to fit a Bayesian   adaptive spline surface to a dataset that either follows a Student's t-distribution or has outliers. Much of the framework for TBASS is adapted from the concepts of Bayesian Multivariate Adaptive Regression Splines (BMARS), specifically the work done by Denison, Mallick, and Smith (1998). By including a more robust generalization, a dataset with outliers can now be accurately fit using the BMARS model, without the possibility of overfitting or variance inflation.\newline
...
\begin{equation}
\label{lamprior}
\lambda \sim \Gamma \bigg(h_1, ~h_2\bigg)
\end{equation}
...
\begin{equation}
\label{8}
\boldsymbol\beta | \cdot \sim \mathcal{N} \left( 0,~\tau^2 {\bf I} \right)
\end{equation}
...
Based on the priors and likelihood, we obtain the full conditional of $V_i$ as
...
where $M$ is the current number of basis functions.
...
Like Gaussian BMARS, the robust algorithm adaptively builds, deletes, and modifies basis functions, sampling from candidate knot locations, signs, interaction degrees, and accepting or rejecting the candidate values using a RJ-MCMC algorithm to sample from the full posterior.
...
where $D$ represents the data, $\theta'$ represents the candidate model parameters and $\theta$ represent the current state model parameters, $L$ is the likelihood, $p$ is the prior, and $S$ is the proposal to jump from one model to another. 
...
\begin{equation}
\label{12}
\frac{L(y \mid {\theta' = M'})}{L(y \mid {\theta = M})} = \frac{\left(\tau^2\right)^{-\frac{M+2}{2}}  |{\bf V}|^{-1/2}~ \left|{\bf H}^{-1}_c\right|^{-1/2} \exp\left\lbrace-\frac{1}{2}\left({\bf y'V^{-1}y} - \boldsymbol{\hat{\beta}}'_{c} \left({\bf X'_{c}V^{-1}X_{c}} + \tau^{-2}{\bf I_{c}}\right) \boldsymbol{\hat\beta}_{c}\right)\right\rbrace}
...
Once the basis function move type is complete, the model parameter values $\lambda$, $V_i$, $\sigma^2$, and $\boldsymbol{\beta}$ can then be sampled using Gibbs Sampling, since the full conditionals are all closed-form (@denison1998bayesian). The Gibbs Sampling steps is not unique to the birth step as they are performed after every RJ-MCMC iteration.
...
From there, we can run the `tbass()` command with 20000 MCMC iterations (default 10000). Our first simulation is with $\nu = 10$ degrees of freedom to simulate a t-distribution with thicker tails.
...
From there, we plot our $\sigma$ values without the burn-in to ensure there was no variance inflation in Figure 5.
...
We constructed a generalized version of Gaussian Bayesian Multivariate Adaptive Regression to accommodate data with outliers. This framework provides powerful and accurate parameter estimation. The R package `TBASS` adopts this framework from the original `BASS` package. These two model has been tested and compared to show their differences and demonstrate how a low value of $\nu$ can emulate the results from a Student's t-distribution.     
