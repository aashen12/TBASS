...
tbass <- function(X, y, max.int=3, max.basis=50, tau2=10^4, nu=10, nmcmc=10000,
                  g1=0, g2=0, h1=10, h2=10, verbose=FALSE) {
  ticker <- nmcmc / 10
  ...
  for(i in 2:nmcmc) {
    ...
    if(move.type == 'birth'){
      ...
      vars.cand <- sample(p, nint.cand, replace = FALSE) # variables to use in new basis function
      ...
      X.cand <- X.curr[, -(tokill + 1), drop=FALSE] # +1 to skip the intercept
      d.cand <- getdC(X.cand,v[i-1,], s2[i-1], tau2, y)
    ...
    res <- y - X.curr %*% t(beta[i, 1:(nbasis[i]+1), drop=FALSE])
    ...
    if(verbose == TRUE) {
      if(i %% ticker == 0) {
        cat(timestamp(quiet = TRUE),' nmcmc: ', i, ' nbasis: ', nbasis[i], '\n')
      }
    }
  }
